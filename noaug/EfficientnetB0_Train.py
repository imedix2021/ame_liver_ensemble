...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("efficientnetB0_-20_256_liver_20220312_rsf.txt", "w")
...
with strategy.scope():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
        ...
        model_checkpoint = ModelCheckpoint(
            weights_dir + "val_loss{val_loss:.3f}_efficientnetB0_-20_256_liver_20220312_rsf.hdf5",
            monitor = 'val_loss',
            verbose = 1,
            save_best_only = True,
...
def model_save(model_name):
    ...
    with open(model_dir +"Amed_Liver_-20_256_20220312" + model_name + "_rsf_noisy.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'Amed_Liver_-20_256_20220312' + model_name + '_rsf_noisy.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("efficientnetB0")
save_history(hist, os.path.join(result_dir, 'history_efficientnetB0_-20_256_liver_20220312_rsf.txt'))
learning_plot("efficientnetB0_-20_256_liver_20220312_rsf")
