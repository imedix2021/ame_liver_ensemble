...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("ResNeXt101_all_256_liver_noaug_20220423.txt", "w")
...
with strategy.scope():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
        ...
        model_checkpoint = ModelCheckpoint(
            weights_dir + "val_loss{val_loss:.3f}_ResNeXt101_all_256_liver_noaug_20220423.hdf5",
            monitor = 'val_loss',
            verbose = 1,
            save_best_only = True,
        ...
        base_model = ResNeXt101(include_top = False, input_shape=(256, 256, 3), weights='imagenet')
        '''
...
def model_save(model_name):
    ...
    with open(model_dir +"Liver_256_noaug" + model_name + "_all_20220423_rsf_batch32.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'Liver_256_noaug' + model_name + '_all_20220423_rsf_batch32.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("ResNeXt101")
save_history(hist, os.path.join(result_dir, 'history_ResNeXt101_all_256_liver_noaug_20220423_rsf_batch32.txt'))
learning_plot("ResNeXt101_all_256_liver_noaug_20220423_rsf_batch32")
