...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("ResNet50_all_liver_noaug_20210727_rsf_batch32_size224.txt", "w")
...
with strategy.scope():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
        ...
        model_checkpoint = ModelCheckpoint(
            weights_dir + "val_loss{val_loss:.3f}_ResNet50_all_liver_noaug_20210727_rsf_batch32_size256.hdf5",
            monitor = 'val_loss',
            verbose = 1,
            save_best_only = True,
...
def model_save(model_name):
    ...
    with open(model_dir +"liver_noaug" + model_name + "_20210727_rsf_batch32_size256.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'liver_noaug' + model_name + '_20210727_rsf_batch32_size256.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("ResNet50")
save_history(hist, os.path.join(result_dir, 'history_ResNet50_all_liver_noaug_20210727_rsf_batch32_size256.txt'))
learning_plot("ResNet50_all_liver_noaug_20210727_rsf_batch32_size256")
