...
#from tensorflow.keras.applications.resnet import ResNeXt50from keras.models import Model, load_model
# for keras
import keras
...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("ResNeXt50_all_256_liver_noaug_rsf.txt", "w")
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
    ...
    model_checkpoint = ModelCheckpoint(
        weights_dir + "val_loss{val_loss:.3f}_ResNeXt50_all_256_codid_noaug_rsf.hdf5",
        monitor = 'val_loss',
        verbose = 1,
        save_best_only = True,
    ...
    #base_model = ResNeXt50(include_top = False, input_shape=(512, 512, 3), weights='imagenet')
    input_tensor = Input(shape=(image_size, image_size, 3))
    base_model = ResNeXt50(include_top=False, weights='imagenet',input_tensor=input_tensor)
...
def model_save(model_name):
    ...
    with open(model_dir +"liver_256_noaug" + model_name + "_20210728_rsf.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'liver_256_noaug' + model_name + '_20210728_rsf.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("ResNeXt50_all")
save_history(hist, os.path.join(result_dir, 'history_ResNeXt50_all_liver_256_noaug_rsf.txt'))
learning_plot("ResNeXt50_all_liver_256_noaug_rsf")
