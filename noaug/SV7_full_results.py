...
np.save('y_pred_soft7.npy', y_pred_soft7)
...
#np.savetxt('correctLiver_EnsembleSoft7_20220318_rsf.csv',true_classes,delimiter=',')

# testデータ n件の予測ラベル
#pred_classes = np.argmax(model.predict(X_test[0:n]), axis = 1)
averaging_predict7 = []
averaging_predict7 = (  model1.predict(X_test[0:1000]) + 
...
# 書き込み
#np.save('first_probs2.npy', first_probs)
np.save('averaging_predict7.npy', averaging_predict7)

'''
# 読み込み
#first_probs = np.load ('first_probs2.npy')
averaging_predict7  = np.load ('averaging_predict7.npy')
y_pred_soft7= np.load ('y_pred_soft7.npy')


pred_classes = np.argmax(averaging_predict7, axis = 1)
print('prediction:', pred_classes)


print('4x4_EnsembleSoft7_liver_noaug_Results_2022059_Final_New')

cf_matrix = confusion_matrix(np.argmax(y_test, 1), np.argmax(y_pred_soft7, 1))
...
print(classification_report(np.argmax(y_test, 1), np.argmax(y_pred_soft7, 1), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']))

# testデータ n件の正解ラベル
#true_classes = np.argmax(y_test[0:n], axis = 1)
true_classes = np.argmax(y_test[0:1000], axis = 1)
#print('correct:', true_classes)
np.savetxt('correctLiver_EnsembleSoft7_liver_noaug_rsf_batch32_size256_New.csv',true_classes,delimiter=',')

# testデータ n件の予測ラベル
#pred_classes = np.argmax(model.predict(X_test[0:n]), axis = 1)
pred_classes = np.argmax(averaging_predict7, axis = 1)
#print('prediction:', pred_classes)
np.savetxt('predictionsLiver_EnsembleSoft7_liver_noaug_rsf_batch32_size256_New.csv',pred_classes,delimiter=',')
...
plt.savefig('/home/USER/amed_liver/noaug/EnsembleSoft7_noaug_roc_curve1.png')
plt.gca().clear()
...
plt.legend()
#Replace USER with the user name of the computer you are using.
plt.savefig('/home/USER/amed_liver/noaug/EnsembleSoft7_roc_curve2_macro.png')
plt.gca().clear()
...
plt.legend()
#Replace USER with the user name of the computer you are using.
plt.savefig('/home/USER/amed_liver/noaug/EnsembleSoft7_roc_curve3_both.png')
plt.gca().clear()
...
plt.legend()
plt.savefig('/home/USER/amed_liver/noaug/IEnsembleSoft7_roc_curve4_micro.png')
#Replace USER with the user name of the computer you are using.
...
plt.savefig('EnsembleSoft7_liver_Heatmap_20220423_final_rsf.png')
