...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("InceptionvResNetV2_256_liver_20210726.txt", "w")
...
with strategy.scope():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
        ...
        model_checkpoint = ModelCheckpoint(
            weights_dir + "val_loss{val_loss:.3f}_IRNV2_256_liver_20210726_rsf.hdf5",
            monitor = 'val_loss',
            verbose = 1,
            save_best_only = True,
...
def model_save(model_name):
    ...
    with open(model_dir +"Amed_Liver_256_775_20210726" + model_name + "_rsf.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'Amed_Liver_256_775_20210726' + model_name + '_rsf.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("InceptionResNetV2")
save_history(hist, os.path.join(result_dir, 'history_inceptionresnetv2_775_256_liver_20210726_rsf.txt'))
learning_plot("InceptionV3_249_liver_20210726_rsf_batch32_size299")
