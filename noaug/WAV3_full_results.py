...
def Voting(x_voting,y_voting,k1,k2,k3):
    ...
    voting_result  = (k1*model1_predict + k2*model2_predict + k3*model3_predict)/(k1+k2+k3)
...
def f(x):
    ...
    score = -Voting(X_train_n,y_train_ichibu,k1,k2,k3)
...
f.write('Bayes WAV 6: %s, Score: %.3f' % (myBopt.x_opt,  Voting(X_test,y_test2,k_best[0],k_best[1],k_best[2])))
f.write(' \n')
...
#WA_predict3  = np.load ('WA_predict3.npy')
#WA_predict3 = np.load ('WA_predict3.npy')
WA_predict3 = []

WA_predict3 = (k_best[0]*model1.predict(X_test, verbose=1) + k_best[1]*model2.predict(X_test, verbose=1) + k_best[2]*model3.predict(X_test, verbose=1))/(k_best[0]+k_best[1]+k_best[2])

# 書き込み
#np.save('first_probs3.npy', first_probs)
np.save('WA_predict3.npy', WA_predict3)

pred_classes = np.argmax(WA_predict3, axis = 1)
#print('prediction:', pred_classes)


print('4x4_WAV3_liver_noaug_Results')

cf_matrix = confusion_matrix(np.argmax(y_test, 1), np.argmax(WA_predict3, 1))
...
print(classification_report(np.argmax(y_test, 1), np.argmax(WA_predict3, 1), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']))
...
np.savetxt('correctLiver_WAV3_liver_noaug_rsf_batch32_size256_New.csv',true_classes,delimiter=',')

# testデータ n件の予測ラベル
#pred_classes = np.argmax(model.predict(X_test[0:n]), axis = 1)
pred_classes = np.argmax(WA_predict3, axis = 1)
#print('prediction:', pred_classes)
np.savetxt('predictionsLiver_WAV3_liver_noaug_rsf_batch32_size256_New.csv',pred_classes,delimiter=',')
...
plt.savefig('/home/USER/amed_liver/noaug/WAV3_roc_curve1.png')
plt.gca().clear()
...
plt.legend()
#Replace USER with the user name of the computer you are using.
plt.savefig('/home/USER/amed_liver/noaug/WAV3_roc_curve2_macro.png')
plt.gca().clear()
...
plt.legend()
#Replace USER with the user name of the computer you are using.
plt.savefig('/home/USER/amed_liver/noaug/WAV3_roc_curve3_both.png')
plt.gca().clear()
...
plt.legend()
plt.savefig('/home/USER/amed_liver/noaug/WAV3_roc_curve4_micro.png')
#Replace USER with the user name of the computer you are using.
...
plt.savefig('WAV3_Heatmap_20220423_final_rsf.png')
...
f = open('WAV3_Full_Final_Result_amed_liver.txt', 'w')
f.write('WAV3\n')
f.write(classification_report(np.argmax(y_test, 1), np.argmax(WA_predict3, 1), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']) + '\n')
f.write(' \n')
