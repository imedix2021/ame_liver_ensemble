...
def Voting(x_voting,y_voting,k1,k2,k3,k4,k5,k6):
    ...
    voting_result  = (k1*model1_predict + k2*model2_predict + k3*model3_predict + k4*model4_predict +k5*model5_predict + k6*model6_predict)/(k1+k2+k3+k4+k5+k6)
...
print("Normal Voting")
_ = Voting(X_test,y_test,1,1,1,1,1,1)
...
def f(x):
    ...
    score = -Voting(X_train_n,y_train_ichibu,k1,k2,k3,k4,k5,k6)
...
#結果出力
print("best [k1 k2 k3 k4 k5 k6] =")
print(myBopt.x_opt)
print("Weighted Voting")
k_best = myBopt.x_opt
_ = Voting(X_test,y_test,k_best[0],k_best[1],k_best[2],k_best[3],k_best[4],k_best[5])

t = time.time() - start
print('WSV time -  reading models:', t, 'sec')

f = open('Weighted Averaging_Voting_Best6_Bayesian_Result_amed_liver.txt', 'w')
f.write('Best6_Weighted_Averaging_SoftVoting\n')
...
f.write('Bayes WAV 6: %s, Score: %.3f' % (myBopt.x_opt,  Voting(X_test,y_test2,k_best[0],k_best[1],k_best[2],k_best[3],k_best[4],k_best[5])))
f.write(' \n')
...
#WA_predict6  = np.load ('WA_predict6.npy')
#WA_predict6 = np.load ('WA_predict6.npy')
WA_predict6 = []

WA_predict6 = (k_best[0]*model1.predict(X_test, verbose=1) + k_best[1]*model2.predict(X_test, verbose=1) + k_best[2]*model3.predict(X_test, verbose=1) 
+  k_best[3]*model4.predict(X_test, verbose=1)+k_best[4]*model5.predict(X_test, verbose=1) + k_best[5]*model6.predict(X_test, verbose=1))/(k_best[0]+k_best[1]+k_best[2]+k_best[3]+k_best[4]+k_best[5])

# 書き込み
#np.save('first_probs3.npy', first_probs)
np.save('WA_predict6.npy', WA_predict6)

pred_classes = np.argmax(WA_predict6, axis = 1)
#print('prediction:', pred_classes)


print('4x4_WAV6_liver_noaug_Results')

cf_matrix = confusion_matrix(np.argmax(y_test, 1), np.argmax(WA_predict6, 1))
...
print(classification_report(np.argmax(y_test, 1), np.argmax(WA_predict6, 1), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']))
...
np.savetxt('correctLiver_WAV6_liver_noaug_rsf_batch32_size256_New.csv',true_classes,delimiter=',')

# testデータ n件の予測ラベル
#pred_classes = np.argmax(model.predict(X_test[0:n]), axis = 1)
pred_classes = np.argmax(WA_predict6, axis = 1)
#print('prediction:', pred_classes)
np.savetxt('predictionsLiver_WAV6_liver_noaug_rsf_batch32_size256_New.csv',pred_classes,delimiter=',')
...
plt.savefig('/home/USER/amed_liver/noaug/WAV6_roc_curve1.png')
plt.gca().clear()
...
plt.legend()
#Replace USER with the user name of the computer you are using.
plt.savefig('/home/USER/amed_liver/noaug/WAV6_roc_curve2_macro.png')
plt.gca().clear()
...
plt.legend()
#Replace USER with the user name of the computer you are using.
plt.savefig('/home/USER/amed_liver/noaug/WAV6_roc_curve3_both.png')
plt.gca().clear()
...
plt.legend()
plt.savefig('/home/USER/amed_liver/noaug/WAV6_roc_curve4_micro.png')
#Replace USER with the user name of the computer you are using.
...
plt.savefig('WAV6_Heatmap_20220423_final_rsf.png')
...
f = open('WAV6_Full_Final_Result_amed_liver.txt', 'w')
f.write('WAV6\n')
f.write(classification_report(np.argmax(y_test, 1), np.argmax(WA_predict6, 1), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']) + '\n')
f.write(' \n')
