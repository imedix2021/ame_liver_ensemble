...
def Voting(x_voting,y_voting,k1,k2,k3):
    ...
    #model16_predict = np.argmax(model16.predict(x_voting), axis = 1) 
    ...
    #model16_onehot = to_categorical(model16_predict, 10)
    ...
    for i in range(len(x_voting)):
        ...
        voting = np.argmax(k1*model1_onehot[i] +
                            ...
                            k4*model2_onehot[i] + k5*model3_onehot[i] +
                            k6*model2_onehot[i] + k7*model3_onehot[i] +
                            k8*model2_onehot[i] + k9*model3_onehot[i] +
                            k10*model2_onehot[i] + k11*model3_onehot[i] +
                            k12*model2_onehot[i] + k13*model3_onehot[i] +
                            k14*model2_onehot[i] + k15*model3_onehot[i])
        '''
        voting = np.argmax(k1*model1_onehot[i] +
                            k2*model2_onehot[i] + k3*model3_onehot[i] )
...
acc = accuracy_score(y_test2, Voting(X_test,y_test2,1,1,1))
print('Normal HardVoting acc:', acc)
weighted_acc =  accuracy_score(y_test2, Voting(X_test,y_test2,1.10908312,0.78051241,1.01023778))
print('WeightedHardVoting acc:', weighted_acc)
print(classification_report(y_test2, Voting(X_test,y_test2,1.10908312,0.78051241,1.01023778), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']))
cf_matrix = confusion_matrix(y_test2, Voting(X_test,y_test2,1.10908312,0.78051241,1.01023778))
...
np.savetxt('correctLiver_WeightedHardVoting3.csv',true_classes,delimiter=',')

# testデータ n件の予測ラベル
#pred_classes = np.argmax(model.predict(X_test[0:n]), axis = 1)
pred_classes = Voting(X_test,y_test2,1.10908312,0.78051241,1.01023778)
#print('prediction:', pred_classes)
np.savetxt('predictionsLiver_WeightedHardVoting3.csv',pred_classes,delimiter=',')
...
plt.savefig('WeightedHardVoting3_Heatmap_20220423_final_rsf.png')
...
f = open('WeightedHardVoting3_Full_Final_Result_amed_liver.txt', 'w')
f.write('WeightedHardVoting3_\n')
f.write(classification_report(y_test2, Voting(X_test,y_test2,1.10908312,0.78051241,1.01023778), digits=3, target_names=['BLTumor', 'LCyst', 'MLCancer', 'PLCancer']) + '\n')
f.write(' \n')
...
f.write('Read models time(sec) :' + str(t3) + '\n')
f.close()
