...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("Xception_256_liver_noaug_20210726_rsf.txt", "w")
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
    ...
    model_checkpoint = ModelCheckpoint(
        weights_dir + "val_loss{val_loss:.3f}_Xception_256_liver_noaug_20210726_rsf.hdf5",
        monitor = 'val_loss',
        verbose = 1,
        save_best_only = True,
...
def model_save(model_name):
    ...
    with open(model_dir +"Liver_256_noaug" + model_name + "_20210726_rsf.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'Liver_256_noaug' + model_name + '_20210726_rsf.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("Xception")
save_history(hist, os.path.join(result_dir, 'history_Xception_256_liver_noaug_20210726_rsf.txt'))
learning_plot("Xception_256_liver_noaug_20210726_rsf")
