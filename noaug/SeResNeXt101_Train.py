...
#from tensorflow.keras.applications.resnet import SeResNeXt101from keras.models import Model, load_model
# for keras
import keras
...
def model_evaluate():
   #score = model.evaluate(X_test, y_test, verbose = 1)
    ...
    fileobj = open("SeResNet50_all_256_liver_noaug_20220419_rsf_batch32.txt", "w")
...
with strategy.scope():
    ...
    for fold, (train_index, val_index) in enumerate(kf.split(X_train,y_train)):
        ...
        model_checkpoint = ModelCheckpoint(
            weights_dir + "val_loss{val_loss:.3f}_SeResNeXt101_all_256_codid_noaug_20220419_rsf_batch32.hdf5",
            monitor = 'val_loss',
            verbose = 1,
            save_best_only = True,
        ...
        base_model = SeResNeXt101(include_top = False, input_shape=(256, 256, 3), weights='imagenet')
        '''
...
def model_save(model_name):
    ...
    with open(model_dir +"liver_256_noaug" + model_name + "_all_20220419_rsf_batch32.json", "w") as json_file:
     json_file.write(model_json)

    model.save_weights(weights_dir + 'liver_256_noaug' + model_name + '_all_20220419_rsf_batch32.h5')
    # optimizerのない軽量モデルを保存（学習や評価不可だが、予測は可能）

model_save("SeResNeXt101")
save_history(hist, os.path.join(result_dir, 'history_SeResNeXt101_all_256_liver_noaug_20220419_all_rsf_batch32.txt'))
learning_plot("SeResNeXt101_all_liver_256_noaug_20220419_rsf_batch32")
